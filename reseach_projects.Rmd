---
title: Collaborations, Network & Research Projects
---
...
### 2. Summary of the projects {.tabset .tabset-pills}
...
vertical axis indicates the number of updates accumulated over the optimization. Higher values on the
...
![Anytime HV (higher is better) performance of MOEA/D-PS for different values of **ps** on DTL6. MOEA/D-PS  defines  an  expected amount of solutions updated at each iteration, regulated by a control  parameter, **ps**.](imgs/DTLZ6hv_evolution.png)

Future works also include finding ways to improve the performance of MOEA/D on simulated real-world problems using MOEA/D variants that use resource allocation (RA)  \cite{lavinas2019using,lavinas2019improving,lavinas2020moea} since they show promising results in MOP without constraints.
...
![Anytime mean hypervolume (HV, higher is better) performance of the different CHT in MOEA/D in the MAZDA car problem. Our newly proposed Constraint Handling Technique, the Three Stage CHT, achieves the best values at the end of the search execution.](imgs/MAZDA_HV.png)


![Anytime mean HV (higher is better) performance of the different CHT in MOEA/D in the moon landing problem. The Three Stage CHT has the competitive performance with most of the other CHT, but is overcomed by the Fast growth Dynamic CHT.](imgs/MOON_HV.jpg)
...
![Example of real output of a game where one player is controlled by an Artificial NeuralNetwork using the Proximal Policy Optimization](imgs/2D_representation_video.png)
...
![Spread time series for teams A and B in a game style where one player is controlled by an Artificial Neural Network using the Proximal Policy Optimization.](imgs/spread-NN-full.png)
